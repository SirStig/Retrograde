...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordHeader") {
			username = "SirStig"
			avatarUrl = "https://github.com/SirStig.png"
			webhookUrl = webhook
			if (!isDev) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [Retrograde version `$version` is out!](https://modrinth.com/mod/retrograde/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [Retrograde version `$version` is out!](https://modrinth.com/mod/retrograde-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
